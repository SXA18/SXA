...
def contact():
	os.system('xdg-open https://www.facebook.com/profile.php?id=100069763880640')
	back()
def animation(u):
...
def info():
 ...
 Author    : SXA BROS
 Github    : SXA18
 Facebook  : Ɣʬ'ɽ Aɭɘx Xʬɗ Ȭʬɭɭɘ'x
 Username  : ፨⃝𝙏͜𝙝'𝙬『𝙎𝙪𝙟𝙖͢͡𝙣』'𝙭輪 ‎
 Username  : 𝐓͜𝐡́͡𝐰 𝐀𝐥͜͡ɚ𝐱 ⸙蒙
\033[1;37m----------------------------------------------""")
...
def approval(): 
  ...
  print ("""\033[1;37m----------------------------------------------""")
  try:
    httpCaht = requests.get("https://github.com/SXA18/SXA/blob/main/approval.txt").text
    if id in httpCaht:
...
def passu():
	...
	linex()
	prog = Progress(TextColumn(' [SXA-XD]{task.description}'))
